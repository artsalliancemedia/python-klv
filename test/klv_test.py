...
class TestKLV(unittest.TestCase):
    ...
    def test_encode_ber(self):
        self.assertEqual(klv.encode_ber(0, 4), [0x83, 0x00, 0x00, 0x00])
        self.assertEqual(klv.encode_ber(10, 4), [0x83, 0x00, 0x00, 0x0a])
        self.assertEqual(klv.encode_ber(127, 4), [0x83, 0x00, 0x00, 0x7f])
        self.assertEqual(klv.encode_ber(255, 4), [0x83, 0x00, 0x00, 0xff])
        self.assertEqual(klv.encode_ber(256, 4), [0x83, 0x00, 0x01, 0x00])
        self.assertEqual(klv.encode_ber(65535, 4), [0x83, 0x00, 0xff, 0xff])
        self.assertEqual(klv.encode_ber(65536, 4), [0x83, 0x01, 0x00, 0x00])
        self.assertEqual(klv.encode_ber(65546, 4), [0x83, 0x01, 0x00, 0x0a])
        self.assertEqual(klv.encode_ber(65536 + 256, 4), [0x83, 0x01, 0x01, 0x00])
        self.assertEqual(klv.encode_ber(65536 + 257, 4), [0x83, 0x01, 0x01, 0x01])
    ...
    def test_encode_klv_key_length_16(self):
            ...
            self.assertEqual(k, key + [0x05] + value)
            k = klv.encode(key, value, 4);
            self.assertEqual(k, key + [0x83, 0x00, 0x00, 0x05] + value)
